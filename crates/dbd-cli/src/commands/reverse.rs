...
pub async fn cmd_merge(
    ...
    // Load the existing project's config once: its `format` settings drive the
    // emit-side formatting (so generated bytes match `dbd format`), and its
    // declared schemas drive the undeclared-schema warning below. Reusing the
    // single read avoids a second parse of design.yaml.
    ...
    let conn = resolve_conn(candidate)?;
    let covered = run(
        project_dir, &conn, None, None, sel, force, dry_run, false, &config.format,
    )
    .await?;
...
async fn run(
    ...
    // 3. build write-plan (emitted DDL is formatted so it matches `dbd format`)
