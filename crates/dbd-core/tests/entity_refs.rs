...
fn one(sql: &str) -> Entity {
    parse_sql(sql).expect("reads").entities.into_iter().next().expect("an entity")
}
...
fn a_routine_reports_each_reference_with_its_direction_and_provenance() {
    ...
    let find = |name: &str| e.refs.iter().find(|r| r.name == name).unwrap_or_else(|| panic!("{name}: {:?}", e.refs));
...
fn a_table_both_read_and_written_is_two_rows_not_two_duplicates() {
    ...
    let mut kinds: Vec<RefKind> = e.refs.iter().filter(|r| r.name == "app.audit").map(|r| r.kind).collect();
...
fn a_role_membership_is_not_a_read() {
    let e = one("create role advanced in role basic;");
