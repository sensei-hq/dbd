...
window.DiagramLayout = (function () {
  ...
  function visibleCols(table, density) {
  ...
  function compute(data, density, arrange = "untangle") {
    ...
    const clusters = schemaNames.map((s, i) => ({
      name: s,
    ...
    function pack(c) {
      ...
      c.h = Math.max.apply(null, colH) - GAP_Y + CL_PAD * 2 + CL_TITLE;
    }
    ...
    if (arrange === "untangle" && clusters.length > 1) {
      ...
      const lk = (a, b) => links[a.name + "|" + b.name] || 0;
      const left = clusters.slice().sort((a, b) => b.w * b.h - a.w * a.h);
      ...
      clusters.length = 0;
      Array.prototype.push.apply(clusters, ordered);
    } else {
    ...
    function flow() {
      ...
      for (const c of clusters) {
        if (x > 0 && x + c.w > MAX_ROW_W) { x = 0; y += rowH + CL_GAP_Y; rowH = 0; }
        c.x = x; c.y = y;
        x += c.w + CL_GAP_X;
      ...
      return {
        w: Math.max.apply(null, clusters.map((c) => c.x + c.w)) + 60,
        h: y + rowH + 60,
      };
    }
    let size = flow();
    ...
    data.refs.forEach((r, i) => {
      ...
      let s1, s2; // 1 = right side, -1 = left side
